...
class Transformer(nn.Module):
    ...
    def forward(self, input, t, text_input, padding_masks=None, return_res=False):
        emb = self.emb(input)
        if text_input is not None:
            text_input = self.proj_text(text_input)
        else:
            text_input = None
        inp_enc = self.pos_enc(emb)
        enc_cond = self.encoder(inp_enc, t, padding_masks=padding_masks, label_emb=text_input)

        inp_dec = self.pos_dec(emb)
        output, mean, trend, season = self.decoder(inp_dec, t, enc_cond, padding_masks=padding_masks, label_emb=text_input)
...
class TrendTransformer(nn.Module):
    def __init__(self, args):
        ...
        # patchify
        self.channel=1
        self.H = args.num_nodes #6 # 30
        self.W = 96
        emb_size=768 #64
        self.patch_size=2
        self.Transformer = Transformer(n_feat=args.num_nodes, n_channel=96, n_layer_enc=2, n_layer_dec=3, n_embd=128, n_heads=4, max_len=96)
    ...
    def forward(self, input: torch.Tensor, t: torch.Tensor, text_input):
        ...
        # x = x + self.pos_embed
        trend, season_error = self.Transformer(input, t, text_input)
...
if __name__ == '__main__':
    batch_size, seq_len, n_feat = 2, 64, 30
    
    model = Transformer(n_feat=n_feat, n_channel=seq_len, n_layer_enc=2, n_layer_dec=3, n_embd=128, n_heads=4, max_len=64)
    
    input_tensor = torch.randn(batch_size, seq_len, n_feat)
    timestep = torch.randint(0, 1000, (batch_size,))
    
    print(f"Input: {input_tensor.shape}")
    print(f"Timestep: {timestep.shape}")
    
    trend, season_error = model(input_tensor, timestep)
    print(f"Output trend: {trend.shape}")
    print(f"Output season_error: {season_error.shape}")
