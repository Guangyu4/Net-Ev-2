...
class VerbalTS(nn.Module):
    def __init__(self, args, inputdim=1):
        super().__init__()
        self.args = args

        config = {
            "n_var": args.num_nodes,
            "var_dep_type": "independent",
            "channels": 64,
            "multipatch_num": 2,
            "num_steps": args.total_step,
            "diffusion_embedding_dim": 64,
            "device": args.device,
            "side": {
                "num_var": args.num_nodes,
                "var_emb": 64,
                "time_emb": 64
            },
        ...
        self.text_projector = nn.Linear(768, 64)
        self.config = config
    ...
    def forward(self, x_raw, tp, attr_emb_raw, diffusion_step):
        ...
        if attr_emb_raw is None:
            ...
            if "scale" in self.config["text_projector"]:
                ...
                attr_emb_raw = self.text_projector(attr_emb_raw).unsqueeze(2)  # [32, 64, 1]
                attr_emb_raw = attr_emb_raw.repeat(1, 1, n_var).unsqueeze(1).permute(0,3,1,2)  # [32, 64, n_var]
        ...
        all_out = all_out.reshape((B_raw, n_var, L)).transpose(1, 2)
        return all_out, {}
...
if __name__ == "__main__":
    ...
    args = Args()
    model = VerbalTS(args, inputdim=1)
    
    B, n_var, L = 2, 4, 64
    x_raw = torch.randn(B, 1, n_var, L)
    tp = torch.randint(0, L, (B, L))
    attr_emb_raw = torch.randn(B, n_var, 1, 64)
    diffusion_step = torch.randint(0, 1000, (B,))
    
    output, info = model(x_raw, tp, attr_emb_raw, diffusion_step)
    # B,1,N,T
    # B,T
    # B,N,1,E
    # B
