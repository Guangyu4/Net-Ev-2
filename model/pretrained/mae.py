...
class mae(BaseModel):
    ...
    def shared_eval(self, batch, optimizer, mode):
        if mode == 'train':
            ...
            tem_batch = batch.clone()
            batch = batch * tem_mask
            z = self.encoder(batch.unsqueeze(-1).transpose(1,2)) #(B,T,N)
            data_recon_tem = self.decoder(z).squeeze(-1).transpose(1,2)
            tem_z = data_recon_tem.clone()


            data_recon_tem = data_recon_tem * spa_mask
            z = self.encoder(data_recon_tem.unsqueeze(-1).transpose(1,2)) #(B,T,N)
            data_recon_spa = self.decoder(z).squeeze(-1).transpose(1,2)

            data_recon = tem_z + data_recon_spa + tem_batch
