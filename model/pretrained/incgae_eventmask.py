...
class incgae_eventmask(BaseModel):
    """Event-mask variant: use event mask (inc_mask) instead of temporal and spatial masks"""
    def __init__(self, args):
        super().__init__()
    ...
    def shared_eval(self, batch, optimizer, mode, nodeid, timeid):
        if mode == 'train':
            ...
            optimizer.zero_grad()
            inc_mask = self.get_inc_mask(batch, nodeid, timeid)
            batch_masked = batch * inc_mask
    ...
    def get_inc_mask(self, x, nodeid, timeid):
        ...
        nodeid_flat = [item for sublist in nodeid for item in sublist if item != -1]
        if len(nodeid_flat) > 0:
            for nid in nodeid_flat:
                if 0 <= nid < N:
                    mask[:, :, nid] = 0
        if len(timeid) > 0 and max(timeid) > 0:
            max_t = min(max(timeid), L)
            mask[:, :max_t, :] = 0
        return mask
