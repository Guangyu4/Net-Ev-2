...
from datafactory.stdataset import STDataset
import torch
import numpy as np
...
class AlternatingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(dataset_idx, np.ndarray):
            dataset_idx = int(dataset_idx.item())
        elif not isinstance(dataset_idx, int):
    ...
    if args.mix_train:
        ...
        if args.dataset_name in ['ETTh1', 'ETTm1','traffic', 'airquality', 'exchangerate', 'weather', 'electricity', 'nationalillness', 'weather']:
            ...
            if args.dataset_name == 'SUSHI':
                ...
                dataset = AlternatingDataset(dataset1, dataset2, dataset3)
            elif args.dataset_name in ['SD', 'GBA', 'GLA', 'CA']:
                dataset = STDataset(name=Data_name, data_root=f'./Data/{args.dataset_name}', period=period,args=args)
            else:
                raise ValueError
...
def custom_collate_fn(batch):
    ...
    def safe_compare(dataset_idx, idx):
        if isinstance(dataset_idx, np.ndarray):
            if dataset_idx.ndim == 0:
                return dataset_idx.item() == idx
            else:
                return np.array_equal(dataset_idx, idx)
        else:
            return dataset_idx == idx
    
    grouped_data = {
        idx: [data for data, dataset_idx in batch if safe_compare(dataset_idx, idx)]
